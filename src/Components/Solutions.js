import React from "react";
import Education from "../assests/sol_img_1.png";
import MarketingAgencies from "../assests/sol_img_2.png";
import SalesTeam from "../assests/sol_img_3.png";
import MarketingTeam from "../assests/sol_img_4.png";
import CustomerSupport from "../assests/sol_img_5.png";
import ECommerce from "../assests/sol_img_6.png";
import HealthCare from "../assests/sol_img_7.png";

function Solutions() {
  return (
    <section className="container-fluid" style={{ backgroundColor: "#E1FDFA" }}>
      <div className="container" id="Education">
        <div className="row pt-5 d-flex">
          <div className="col-md-7 col-12 order-md-1 order-1">
            <div className="row">
              <div className="col-md-10 offset-md-1">
                <p
                  className="mt-5 text-start  fs-6"
                  style={{ color: "#08A391" }}
                >
                  Education | ECS Cloud Infotech
                </p>
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  Meet The Infrastructure Learning Platform
                </h3>
                <p className="pt-3 text-start fs-5">
                  End-to-end,fully customizable CRM solution for growing
                  bussiness and enterprises.
                </p>
              </div>
            </div>
          </div>
          <div className="col-md-5 col-12 order-md-2 d-flex justify-content-center align-items-center">
            <img className="img-fluid" src={Education} alt="education" />
          </div>
        </div>
      </div>
      <div className="container" id="SalesTeam">
        <div className="row mt-5 d-flex">
          <div className="col-md-5 col-12 d-flex justify-content-center align-items-center">
            <img
              className="img-fluid"
              src={MarketingAgencies}
              alt="ecsMarketing"
            />
          </div>
          <div className="col-md-7 col-12">
            <div className="row">
              <div className="col-md-10 offset-md-1">
                <p
                  className="mt-5 text-start  fs-6"
                  style={{ color: "#08A391" }}
                >
                  Award Winning Digital Marketing Agency | ECS Cloud Infotech
                </p>
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  The Digital Marketing Agency That Drives Revenue
                </h3>
                <p className="pt-3 text-start fs-5">
                  Choose ECS Cloud as your digital marketing agency and propel
                  your bussiness to new heights with our award-winning digital
                  marketing services and technology platform.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="container" id="CustomerSupport">
        <div className="row mt-5 d-flex">
          <div className="col-md-7 col-12 order-md-1 order-1">
            <div className="row">
              <div className="col-md-10 offset-md-1">
                <p
                  className="mt-5 text-start fs-6"
                  style={{ color: "#08A391" }}
                >
                  SALES TEAM | ECS Cloud Infotech
                </p>
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  Gather all marketing activities in one place
                </h3>
                <p className="pt-3 text-start fs-5">
                  Streamline your sales pipeline, improve team collaboration,and
                  book meetings with clients whenever they are ready to buy
                </p>
              </div>
            </div>
          </div>
          <div className="col-md-5 col-12 order-md-2 d-flex justify-content-center align-items-center">
            <img className="img-fluid" src={SalesTeam} alt="ecsSalesteam" />
          </div>
        </div>
      </div>
      <div className="container" id="Healthcare">
        <div className="row mt-5 d-flex">
          <div className="col-md-5 col-12 d-flex justify-content-center align-items-center">
            <img
              className="img-fluid"
              src={MarketingTeam}
              alt="ecsMarketingteam"
            />
          </div>
          <div className="col-md-7 col-12">
            <div className="row">
              <div className="col-md-10 offset-md-1">
                <p
                  className="mt-5 text-start  fs-6"
                  style={{ color: "#08A391" }}
                >
                  SMART HR | PAYROLL | hiring | EMPLOYEE TRANSPORT MANAGEMENT |
                  PROJECT MANAGEMENT SYSTEM
                </p>
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  Marketing Team
                </h3>
                <p className="pt-3 text-start fs-5">
                  A Human Resouce Managemnet Systems (HRMS) is a Software
                  Solutions that streamlines and automatesvarious hr functions,
                  including recruitment , employeerecords , payroll , benefits
                  administration , and performance managemnet.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="container" id="MarketingAgencies">
        <div className="row pt-5 d-flex">
          <div className="col-md-7 col-12 order-md-1 order-1">
            <div className="row pt-5">
              <div className="col-md-10 offset-md-1">
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  Customer Support
                </h3>
                <p className="pt-3 text-start fs-5">
                  Leverage our expertise and get a high-performance and scalable
                  mobile application.
                </p>
              </div>
            </div>
          </div>
          <div className="col-md-5 col-12 order-md-2 d-flex justify-content-center align-items-center">
            <img
              className="img-fluid"
              src={CustomerSupport}
              alt="ecsCustomersupport"
            />
          </div>
        </div>
      </div>
      <div className="container" id="MarketingTeams">
        <div className="row pt-5 d-flex">
          <div className="col-md-5 col-12 d-flex justify-content-center align-items-center">
            <img className="img-fluid" src={ECommerce} alt="ecsEcommerce" />
          </div>
          <div className="col-md-7 col-12">
            <div className="row pt-5">
              <div className="col-md-10 offset-md-1">
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  E-Commerce
                </h3>
                <p className="pt-3 text-start fs-5">
                  Leverage our expertise and get a high-performance and scalable
                  mobile application.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="container" id="ECommerce">
        <div className="row pt-5 pb-5 d-flex">
          <div className="col-md-7 col-12 order-md-1 order-1">
            <div className="row">
              <div className="col-md-10 offset-md-1">
                <p
                  className="mt-5 text-start  fs-6"
                  style={{ color: "#08A391" }}
                >
                  HEALTH CARE | ECS Cloud Infotech
                </p>
                <h3
                  className="d-flex text-start display-6 fw-medium"
                  style={{ color: "rgba(0, 0, 0, 0.722)" }}
                >
                  Health Care
                </h3>
                <p className="pt-3 text-start fs-5">
                  Manatal is a comprehensive cloud-based ATS - Applicant
                  Tracking System to make your recruitment process easier.
                  Through effective pipelines, AI-driven insights, and applicant
                  matching, candidate management becomes a seamless task.
                </p>
              </div>
            </div>
          </div>
          <div className="col-md-5 col-12 order-md-2 d-flex justify-content-center align-items-center">
            <img className="img-fluid" src={HealthCare} alt="ecsHealthcare" />
          </div>
        </div>
      </div>
    </section>
  );
}

export default Solutions;
